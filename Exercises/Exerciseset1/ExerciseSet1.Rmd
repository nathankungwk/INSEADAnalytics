...
**Your Answers here:**
AAPL ticket was pulled by the file named dataSet1.R. It was pulled by the array function c("SPY", "AAPL"), then defined into the variable mytickers.

mytickers = c("SPY", "AAPL")
mean(AAPL) [1] 0.001478376

sum(AAPL) [1] 5.954898

sd(AAPL,na.rm=FALSE) [1] 0.02390385

To get Yahoo's Data, we have to modify the array in dataSet1.R to include Yahoo's ticker. The arracy becomes:
...
mean(YHOO) [1] 0.000716737

sd(YHOO, na.rm=FALSE) [1] 0.03020438
...
**Your Answers here:**
The program does not include the first data point. To include 2001-01-01, we must change the start date variable to the day before in file dataSet1.R

startDate = "2000-12-31"

We can get returns of other companies simply by changing the array code. For example, the command below will get you ticker data from Skyworks Solutions, Wells Fargo, and General Mills

mytickers = c("SWKS", "WFC", "GIS") get 
...
**Your Answers here:**
The command counts number of events where there is inconsistency between value in myData to value in StockReturns
