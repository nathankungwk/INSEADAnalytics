...
We have `r nrow(StockReturns)` days of data, starting from `r rownames(StockReturns)[1]` until `r tail(rownames(StockReturns),1)`.  Here are some basic stbyeatistics about the S&P returns:
...
<br>
<br>
1. It is done in the code "mytickers = c("SPY", "AAPL")" in dataSet1.R
<br>
<br>
2.
Cumulative returns:
...
mean(AAPL) [1] 0.001480165

Standard deviation:
sd(AAPL,na.rm=FALSE) [1] 0.02389822
<br>
<br>
3.
To get Yahoo! Data, we have to modify the array in dataSet1.R to include Yahoo's ticker. The arracy becomes:
...
mean(YHOO) [1] 0.0007228056

Standard deviation:

sd(YHOO, na.rm=FALSE) [1] 0.03019898
<hr>
<hr>
...
<br>
<br>
To get number of rows, we use the command:

nrow(transposedData)

To get number of colunmns, we use the command:

ncol(transposedData)
<br>
<br>
We can then use the apply function again to reproduce portfolio from the transposed matrix:
...
<br>
<br>
1. We can change the following in dataSet1.R and run the code again

startDate = "2001-01-01"
<br>
<br>
2. We can get returns of other companies simply by changing the array code. E.g., the command below will give ticker data from Google, Goldman Sachs, and Tesla

mytickers = c("GOOG", "GS", "TSLA") 
portfolio3 = apply(StockReturns,1,mean)
pnl_plot(portfolio3)
<hr>
<hr>
...
The command counts number of rows that are not equal between myData and StockReturns. Since StockReturns has more columns than myData, each row will give an inequality.
<br>
<br>
2. Error message because they have different dimensions.
<hr>
<hr>
### Extra Question
